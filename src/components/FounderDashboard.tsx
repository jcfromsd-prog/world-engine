...
const SovereignDashboard: React.FC<SovereignDashboardProps> = ({ onClose }) => {
    ...
    const [activeTab, setActiveTab] = useState<'pulse' | 'control' | 'flow' | 'identity'>('pulse');

    // Real-Time Identity Grid
    const [identity] = useState(() => ({
        archetype: localStorage.getItem('sovereign_archetype') || 'UNKNOWN',
        cause: localStorage.getItem('sovereign_cause') || 'UNKNOWN',
        squad: localStorage.getItem('sovereign_squad') || 'UNKNOWN',
        bountyCompleted: localStorage.getItem('first_bounty_completed') || 'FALSE'
    }));

    const handleFactoryReset = () => {
        if (confirm('CRITICAL ACTION: This will wipe all local memory and reset the onboarding flow. Proceed?')) {
            localStorage.clear();
    ...
    // Mock Data for Financial Pulse (RESET TO TRUTH)
                ...
                <div className="lg:col-span-3 space-y-6">
                    <div className="border border-green-900 p-4 bg-green-950/5">
                        ...
                        <div className="flex gap-2">
                            {['ACTIVE', 'MAINTENANCE', 'LOCKED'].map((status) => (
                                <button
                                    key={status}
                                    onClick={() => setEngineStatus(status as "ACTIVE" | "MAINTENANCE" | "LOCKED")}
                    ...
                    <nav className="space-y-1">
                        {[
                            ...
                            { id: 'identity', label: '4. Identity Grid', icon: '🆔' },
                        ].map((item) => (
                            <button
                                key={item.id}
                                onClick={() => setActiveTab(item.id as 'pulse' | 'control' | 'flow' | 'identity')}
                    ...
                    <AnimatePresence mode="wait">
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                                    <div className="border border-green-500 p-4 bg-green-900/10">
                                        <div className="text-xs text-green-400 mb-1">SOVEREIGN OPS (10%)</div>
                                        <div className="text-3xl font-bold">${financialPulse.revenue.toLocaleString()}</div>
                                        <div className="text-[10px] text-green-700 mt-2">FOUNDER INCOME</div>
                                    </div>
                                    <div className="border border-green-800 p-4">
                                        <div className="text-xs text-green-600 mb-1">WAR CHEST (10%)</div>
                                        <div className="text-2xl font-bold text-green-700">$0</div>
                                        <div className="text-[10px] text-green-800 mt-2">LEGAL DEFENSE</div>
                                    </div>
                                    <div className="border border-green-800 p-4">
                                        <div className="text-xs text-green-600 mb-1">GROWTH FUND (10%)</div>
                                    ...
                                    <div className="border border-green-800 p-4">
                                        <div className="text-xs text-green-600 mb-1">LEAD SOLVERS (45%)</div>
                                        <div className="text-2xl font-bold text-green-700">${financialPulse.payouts.toLocaleString()}</div>
                                        <div className="text-[10px] text-green-800 mt-2">DIRECT EXECUTION</div>
                                    </div>
                                    <div className="border border-green-800 p-4">
                                        <div className="text-xs text-green-600 mb-1">SUPPORT SQUAD (15%)</div>
                                        <div className="text-2xl font-bold text-green-700">$0</div>
                                        <div className="text-[10px] text-green-800 mt-2">PEER REVIEW</div>
                                    </div>
                                    <div className="border border-green-800 p-4">
                                        <div className="text-xs text-green-600 mb-1">AI COMPUTE (10%)</div>
                                        <div className="text-2xl font-bold text-green-700">${financialPulse.compute.toLocaleString()}</div>
                                        <div className="text-[10px] text-green-800 mt-2">SAGE / LLM COSTS</div>
                                    </div>
                                </div>
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                                    ...
                                    <div className="border border-green-900 p-6 flex flex-col justify-center items-center text-center bg-black">
                                        ...
                                        <button className="px-8 py-4 bg-red-900/20 text-red-500 border border-red-900 hover:bg-red-900/40 uppercase tracking-widest font-bold mb-4">
                                            EXECUTE PARAMETER SWAP
                                        </button>

                                        <button
                                            onClick={handleFactoryReset}
                                            className="px-8 py-2 bg-red-950 text-red-400 border border-red-900 hover:bg-red-900 hover:text-white uppercase tracking-widest font-bold text-xs"
                                        >
                                            [ HARD RESET SYSTEM ]
                                        </button>
                                    </div>
                        ...
                        {activeTab === 'identity' && (
                            <motion.div
                                ...
                                className="relative z-10"
                            >
                                <h2 className="text-2xl mb-8 border-b border-green-800 pb-2">IDENTITY_MATRIX_INSPECTOR</h2>

                                <div className="grid grid-cols-2 gap-8 mb-12">
                                    <div className="p-6 border border-green-500 bg-green-900/10">
                                        <div className="text-xs text-green-600 mb-2">SOVEREIGN_ARCHETYPE</div>
                                        <div className="text-3xl font-bold text-white tracking-widest">{identity.archetype}</div>
                                    </div>
                                    <div className="p-6 border border-green-800 bg-black">
                                        <div className="text-xs text-green-600 mb-2">OPERATIONAL_CAUSE</div>
                                        <div className="text-xl text-green-400 font-mono">{identity.cause}</div>
                                    </div>
                                    <div className="p-6 border border-green-800 bg-black">
                                        <div className="text-xs text-green-600 mb-2">SQUAD_MODE</div>
                                        <div className="text-xl text-green-400 font-mono">{identity.squad}</div>
                                    </div>
                                    <div className="p-6 border border-green-800 bg-black">
                                        <div className="text-xs text-green-600 mb-2">FIRST_PAYCHECK_STATUS</div>
                                        <div className={`text-xl font-mono ${identity.bountyCompleted === 'true' ? 'text-green-400' : 'text-yellow-600'}`}>
                                            {identity.bountyCompleted === 'true' ? 'CLAIMED' : 'PENDING'}
                                        </div>
                                    </div>
                                </div>

                                <div className="border border-green-900 p-6">
                                    <div className="text-xs text-green-700 mb-4">RAW_STORAGE_DUMP</div>
                                    <div className="font-mono text-[10px] text-green-800 whitespace-pre-wrap break-all">

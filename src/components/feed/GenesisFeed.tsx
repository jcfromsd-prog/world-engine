...
import { ApprenticeQueue } from '../../features/marketplace/ApprenticeQueue';
        ...
        <div className="px-0 pb-24 animate-fade-in w-full mt-12 relative">
                ...
                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 flex-wrap">
                    <div>
                        ...
                        <div className="flex items-center gap-4 mt-2">
                            <button
                                onClick={() => setFeedMode('GENESIS')}
                                className={`px-4 py-2 rounded-full font-bold text-xs uppercase transition-all ${feedMode === 'GENESIS' ? 'bg-white text-black' : 'bg-zinc-900 text-zinc-500 hover:text-white'}`}
                            >
                                📡 AI SIMULATION
                            </button>
                            <button
                                onClick={() => setFeedMode('MARKETPLACE')}
                                className={`px-4 py-2 rounded-full font-bold text-xs uppercase transition-all flex items-center gap-2 ${feedMode === 'MARKETPLACE' ? 'bg-emerald-500 text-black' : 'bg-zinc-900 text-zinc-500 hover:text-white'}`}
                            >
                                💼 MARKETPLACE {feedMode !== 'MARKETPLACE' && <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>}
                            </button>
                        </div>
                    </div>

                    {/* FILTERS (Only show in GENESIS mode) */}
                    {feedMode === 'GENESIS' && (
            ...
            {feedMode === 'MARKETPLACE' && (
                <div className="max-w-6xl mx-auto animate-fade-in">
                    <ApprenticeQueue profile={profile} />
                </div>
            )}
